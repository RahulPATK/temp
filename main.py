...
def propose_header_mapping(
    ...
    try:
        ...
        # Full mapping in compact format (one line each, no numbering)
        ...
        return json.dumps({
            ...
            "status": "pending",
            "message": "Review mapping. Say 'approve' or 'reject because <reason>'.",
        }, indent=2)

    except Exception as e:
        print(f"[propose_header_mapping] ❌ Error: {traceback.format_exc()}")
        return json.dumps({"error": str(e), "stage": "header_mapping"}, indent=2)
...
def propose_value_transformation(
    ...
    try:
        ...
        return json.dumps({
            ...
            "status": "pending",
            "message": "Review above. Say 'approve' or 'reject because <reason>'.",
        }, indent=2)

    except Exception as e:
        print(f"[propose_value_transformation] ❌ Error: {traceback.format_exc()}")
        return json.dumps({"error": str(e), "stage": "value_transformation"}, indent=2)
...
def validate_data(
    ...
    try:
        ...
        missing_fields = vr.get("missing_required_fields", [])
        quality_issues = vr.get("data_quality_issues", [])
        return json.dumps({
            "proposal_id": proposal_id,
            "stage": "validation",
            ...
            "data_quality_issues_count": len(quality_issues),
            "data_quality_issues": quality_issues[:10] if quality_issues else [],
        ...
        print(f"[validate_data] ❌ Error: {traceback.format_exc()}")
        return json.dumps({"error": str(e), "stage": "validation"}, indent=2)
...
def propose_mcf_generation(
    ...
    try:
        ...
        # MCF preview (compact — full output is in staging table)
        mcf_lines = mcf_output.split("\n") if mcf_output else []
        # Show header line + first 3 data lines
        mcf_preview = "\n".join(mcf_lines[:4]) if mcf_lines else ""
        if len(mcf_lines) > 4:
            mcf_preview += f"\n... ({len(mcf_lines) - 4} more lines)"

        return json.dumps({
            "proposal_id": proposal_id,
            "stage": "mcf_generation",
            "effective_date": effective_date,
            "mcf_rows": len(mcf_lines),
            "mcf_output_chars": len(mcf_output),
            "mcf_preview": mcf_preview,
            "status": "pending",
            "message": "Review MCF preview. Say 'approve' or 'reject because <reason>'.",
        }, indent=2)

    except Exception as e:
        print(f"[propose_mcf_generation] ❌ Error: {traceback.format_exc()}")
        return json.dumps({"error": str(e), "stage": "mcf_generation"}, indent=2)
...
def approve_proposal(proposal_id: str, stage: str) -> str:
    ...
    try:
        ...
        print(f"[approve_proposal] ❌ Error: {traceback.format_exc()}")
...
def reject_proposal(proposal_id: str, stage: str, feedback: str) -> str:
    ...
    try:
        ...
        print(f"[reject_proposal] ❌ Error: {traceback.format_exc()}")
        return json.dumps({"error": str(e)}, indent=2)
